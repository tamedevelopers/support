...
class WebScraper
{
    ...
    public function __construct(array $config = [], $url = null, $baseUrl = null)
    {
        ...
        if(!empty($url)){
            $this->url = $url;
            $this->baseUrl = $baseUrl ?? self::initParseUrl($url);
        }
        ...
        if(!($this->dom instanceof DOMDocument)) {
            ...
            $this->createEngineOptions($config);
            $this->createDefaultCachePath($config);
            $this->createDefaultSelectors();
        }
    }
    ...
    /**
     * Initialize base URL from the provided URL.
     *
     * @param string $url
     * @return string
     */
    private static function initParseUrl($url)
    {
        return parse_url($url, PHP_URL_SCHEME) . '://' . parse_url($url, PHP_URL_HOST);
    }

    /**
     * Create default selectors for scraping product information.
     */
    private function createDefaultSelectors(): void
    {
    ...
    /**
     * Create default cache path and initialize cache directory if needed.
     * @param array<string, mixed> $config
     */
    private function createDefaultCachePath($config): void
    {
        // Cache configuration
        ...
        if ($this->cacheEnabled && !File::isDirectory($this->cacheDir)) {
     ...
     * @param array<string, mixed> $config
     */
    private function createEngineOptions(array $config): void
    {
