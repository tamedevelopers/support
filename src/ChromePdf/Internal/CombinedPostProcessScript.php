...
final class CombinedPostProcessScript
{
    ...
    public static function asExpression(
        ...
        if ($waitForImages) {
            $js .= "var __allImgs = Array.prototype.slice.call(document.images || []);"
                . "if (__allImgs.length) {"
                .     "await Promise.race(["
                .         "Promise.all(__allImgs.map(function (img) {"
                ...
                .                 "var canDecode = (typeof img.decode === 'function');"
                .                 "if (img.complete && (!img.naturalWidth || img.naturalWidth > 0)) {"
                .                     "if (canDecode) {"
                .                         "try { img.decode().then(finish).catch(finish); return; } catch (e0) {}"
                .                     "}"
                .                     "finish();"
                .                     "return;"
                .                 "}"
                .                 "try { img.addEventListener('load', function () {"
                .                     "if (canDecode) {"
                .                         "try { img.decode().then(finish).catch(finish); return; } catch (e1) {}"
                .                     "}"
                .                     "finish();"
                .                 "}, { once: true }); } catch (e2) {}"
                .                 "try { img.addEventListener('error', finish, { once: true }); } catch (e3) {}"
                .             "});"
                .         "})),"
