...
use Tamedevelopers\Support\ChromePdf\Internal\CloudflareWaitScript;
use Tamedevelopers\Support\ChromePdf\Internal\PriceHydrationWaitScript;
use Throwable;
...
{   
    ...
    private function buildBrowserLaunchConfig(): array
    {
        ...
        if ($this->sourceMode === 'url') {
            ...
            $launch['userAgent'] = ChromiumStealthScript::chromeUserAgent();
        }
    ...
    /**
     * Stealth patches, client hints, and desktop metrics before navigating to a remote URL.
     */
    private function preparePageForUrlNavigation(Page $page): void
    {
        if ($this->sourceMode !== 'url') {
            return;
        }

        $this->applyChromiumStealthToPage($page);
        $this->applyDesktopDeviceMetrics($page);
    }

    private function applyChromiumStealthToPage(Page $page): void
    {
        $session = $page->getSession();

        try {
            $session->sendMessageSync(new Message('Page.addScriptToEvaluateOnNewDocument', [
        ...
        try {
            $session->sendMessageSync(new Message('Emulation.setUserAgentOverride', [
                'userAgent' => ChromiumStealthScript::chromeUserAgent(),
                'acceptLanguage' => 'en-US,en',
                'platform' => 'Win32',
                'userAgentMetadata' => [
                    'brands' => [
                        ['brand' => 'Google Chrome', 'version' => ChromiumStealthScript::chromeMajorVersion()],
                        ['brand' => 'Chromium', 'version' => ChromiumStealthScript::chromeMajorVersion()],
                        ['brand' => 'Not_A Brand', 'version' => '24'],
                    ],
                    'fullVersion' => ChromiumStealthScript::chromeMajorVersion() . '.0.0.0',
                    'platform' => 'Windows',
        ...
        try {
            $page->setExtraHTTPHeaders([
                ...
                'Sec-CH-UA' => '"Google Chrome";v="' . ChromiumStealthScript::chromeMajorVersion() . '", "Chromium";v="' . ChromiumStealthScript::chromeMajorVersion() . '", "Not_A Brand";v="24"',
                'Sec-CH-UA-Mobile' => '?0',
    ...
    private function applyDesktopDeviceMetrics(Page $page): void
    {
        try {
    ...
    /**
     * Wait briefly for Cloudflare / bot interstitials to auto-clear (capped to keep wall time low).
     */
    private function waitForBotChallengeToClear(Page $page, int $maxWaitMs = 8000): void
    {
        if ($this->sourceMode !== 'url') {
            return;
        }

        $evalCap = min(12000, max(2000, $maxWaitMs + 1500));

        try {
            $page->evaluate(CloudflareWaitScript::asExpression($maxWaitMs))->getReturnValue($evalCap);
        } catch (Throwable) {
        }
    }

    private function pageIndicatesBotChallenge(Page $page): bool
    {
        try {
            $result = $page->evaluate(CloudflareWaitScript::isBlockedExpression())->getReturnValue(3000);
    ...
    private function waitForPriceHydration(Page $page, int $maxWaitMs = 5000): void
    {
        if ($this->sourceMode !== 'url') {
            return;
        }

        $evalCap = min(10000, max(2000, $maxWaitMs + 1500));

        try {
            $page->evaluate(PriceHydrationWaitScript::asExpression($maxWaitMs))->getReturnValue($evalCap);
        } catch (Throwable) {
        }
    }
